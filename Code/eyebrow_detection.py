...
while(True):
    ...
    left_cheek_points = list(range(0, 17))
    right_cheek_points = list(range(17, 27)) + list(range(36, 42))
    ...
    for detection in detections:
        ...
        left_cheek = shape[left_cheek_points]
        right_cheek = shape[right_cheek_points]

        #Convert cheek landmarks to integer coordinates for drawing
        left_cheek = left_cheek.astype(int)
        right_cheek = right_cheek.astype(int)

        #Draw left cheek contour
        cv2.polylines(frame, [left_cheek], True, (0, 255, 0), 1)

        #Draw right cheek contour
        cv2.polylines(frame, [right_cheek], True, (0, 255, 0), 1)

        # Extract lip landmarks
        lip = shape[lipBegin:lipEnd]

        # Convert lip landmarks to integer coordinates for drawing
        lip = lip.astype(int)

        # Draw lip contour
        for i in range(0, len(lip) - 1, 2):
            cv2.line(frame, lip[i], lip[i + 1], (0, 255, 0), 1)
